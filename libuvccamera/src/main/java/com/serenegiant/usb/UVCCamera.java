...
public class UVCCamera {
    ...
    public static final int DEFAULT_PREVIEW_WIDTH   = 120;
    ...
    protected long mControlSupports;    // 由Camera控制支持功能標誌
    protected long mProcSupports;       // 由處理單元支持的功能標誌
    ...
    protected int mAnalogVideoStandarMin, mAnalogVideoStandarMax, mAnalogVideoStandarDef;
    ...
    public void open(final USBMonitor.UsbControlBlock ctrlBlock) {
        ...
        nativeConnect(mNativePtr, mCtrlBlock.getVenderId(), mCtrlBlock.getProductId(),
                        mCtrlBlock.getFileDescriptor(), getUSBFSName(mCtrlBlock));
    ...
    public Size getPreviewSize() {
        ...
        for (final Size sz : list) {
            if ((sz.width == mCurrentPreviewWidth) || (sz.height == mCurrentPreviewHeight)) {
                result = sz;
                break;
            }
        }
     ...
     * You can use SurfaceHolder came from SurfaceHolder/GLSurfaceView
     * @param holder
     */
    ...
    public synchronized void resetSharpness() {
        if (mNativePtr != 0) {
            nativeSetSharpness(mNativePtr, mSaturationDef);
        }
    }
    ...
    private static final void dumpControls(final long controlSupports) {
        Log.i(TAG, String.format("controlSupport = %x", controlSupports));
    ...
    private static final void dumpProc(final long procSupports) {
        Log.i(TAG, String.format("procSupport = %x", procSupports));
    ...
    private static final native int nativeSetExposureMode(final long id_camera, final int mExposureMode);
