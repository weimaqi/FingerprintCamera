...
public final class DeviceFilter {
    ...
    // USB device manufacturer name string (or -1 for unspecified)
    ...
    public DeviceFilter(final int vid, final int pid, final int clasz, final int subclass,
        ...
        Log.e(TAG, String.format("VendorId = 0x%04x, ProductId = 0x%04x, Class = %x, Subclass = %x, Protocol = 0x%02x",
    ...
    public DeviceFilter(final UsbDevice device) {
        ...
        mSerialNumber = null;       // device.getSerialName();

        Log.e(TAG, String.format("VendorId = 0x%04x, ProductId = 0x%04x, Class = 0x%02x, Subclass = 0x%02x, Protocol = 0x%02x",
                                    mVendorId, mProductId, mClass, mSubclass, mProtocol));
    }

    /**
     * 從指定的 Xml 資源取得 devicefilter
     * @param context
    ...
    public static DeviceFilter read(final Context context, final XmlPullParser parser) throws XmlPullParserException, IOException {
        int vendorId = -1;
        int productId = -1;
        int deviceClass = -1;
        ...
        while(eventType != XmlPullParser.END_DOCUMENT) {
            ...
            if (!TextUtils.isEmpty(tag) && (tag.equalsIgnoreCase("usb-device"))) {
                if (eventType == XmlPullParser.START_TAG) {
                    ...
                    if (hasValue) {
                        return new DeviceFilter(vendorId, productId, deviceClass, deviceSubClass,
    ...
    public boolean equals(final Object obj) {
        ...
        if (obj instanceof DeviceFilter) {
            ...
            if (filter.mVendorId != mVendorId || filter.mProductId != mProductId || filter.mClass != mClass
                    || filter.mSubclass != mSubclass || filter.mProtocol != mProtocol) {
                return (false);
            }
            ...
            if ((filter.mManufacturerName != null && mManufacturerName != null && !mManufacturerName.equals(filter.mManufacturerName))
                    || (filter.mProductName != null && mProductName != null && !mProductName.equals(filter.mProductName))
                    || (filter.mSerialNumber != null && mSerialNumber != null && !mSerialNumber.equals(filter.mSerialNumber))) {
                return (false);
            }
            return (true);
        }
